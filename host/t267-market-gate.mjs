 ...
 *     → items 34 / counts 11-23-0-34-0/ 三源一致 / 与 `15-requirements-coverage.md` §2 的插件名集合**逐名一致**
 *  14 空集合守卫（一条都没跑 = 红）
 *
...
const REAL_COUNTS = { 'human': 11, evolve: 23, user_space: 0, total: 34, unwired: 0 }
...
try {
  ...
  const realOk = JSON.stringify(real.counts) === JSON.stringify(REAL_COUNTS)
    && real.items.length === 34 && real.inconsistent === false && JSON.stringify(real.differences) === '[]'
  ...
  check('13 真数据正控：真 `host/modules/` + 真 `docs/design/14-plugin-inventory.md`（另一主体维护的真源）'
    + '→ items 34 / counts 11-23-0-34-0/ 三源一致 / 无截断；条目名集合与 `15-requirements-coverage.md` §2 '
