...
const e2e = async ({ weightBps, candidate }) => {
  ...
  const wfiber = await ctx.plugin({
    name: 'webui#e2e', inject: ['ledgerView', 'projection', 'governor', 'observability', 'priceHistory', 'evidenceSummary', 'opsView', 'evolveJournal', 'supplierScorecard'], Config: webuiConfig,
    apply: async (inner, cfg) => {
      const original = inner.provide.bind(inner)
