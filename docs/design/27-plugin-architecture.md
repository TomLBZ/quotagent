...
> **口径真源已移到 `docs/design/29-webui-gui-app.md`**（用户 2026-09-22 指令）：WebUI 是**完整的 GUI 应用**
> （类比 VS Code / DeepSeek harness 的 GUI），**不是**账本投影/只读路由。§6.1 的槽位与路由注册面**保留**，
> 但**注册面必须允许插件提交交互与动作**（视图/区块、表单与校验、动作与命令、业务逻辑钩子、通知与状态），
> 判据也从"页面结构锚点 / 0 内联脚本 / UI 快照 hash"改为"**双方仅用 GUI 走完全部业务流程**"。
> 与 29 冲突时**以 29 为准**；旧判据（UI 快照门、seed 门、ui-mutate 变异门）按 29 §2 删除。
