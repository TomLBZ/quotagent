...
> **口径真源：`docs/design/29-webui-gui-app.md`**（用户 2026-09-22）：WebUI 是**完整 GUI 应用**，
> 不是账本投影/只读路由；注册面还必须允许插件提交**交互与动作**（含服务端一半）。以 29 为准。
...
|---|---|---|
| 槽位（`slot`）：`nav.<view>`、`page.<view>`、`api.<view>`、`admin.<section>` | 一段区块 HTML 生成函数 + 该区块的**只读数据投影（白名单载荷）** + **交互/动作声明（含服务端一半）** | 只知道"有个区块、挂在这个槽位、属于这个前缀" |
| 路由注册：`POST /<prefix>/ui/register`（一次注册，幂等） | `{plugin_id, slot, route, method, auth, what}` | 只做路由表登记与可达性核对（`/api/routes`），不懂 `what` 的业务含义 |
| 静态资源位：`/<prefix>/assets/<plugin_id>/` | 插件自带 CSS/JS/静态资源文件（受信来源：本服务 + 插件注册面；交互脚本允许） | 只按前缀静态服务 |
