...
<!-- 本文件是 `29-webui-gui-app.md` 的归档（预算沿用 `docs/design/*.md` 行 = 28 KB）。规则与口径真源仍是
     主文件（同号小节）；本文件只放**细节**（形状/矩阵/路由/读数），并同受 `tools/check-docs.py` 约束。 -->

为什么有这份文件：主文件受 28 KB 预算，**细节**按仓规的减法顺序（删重复 → 删叙述 → 拆到归档）搬来这里；
主文件的判据与禁止一条未删，搬进来的行逐字保留。
...
1. 工作台首屏第一块面板就是「演示数据（沙盘）」（`webui-sandbox` 贡献，`order:-98`），
   列出可用场景与逐步骤的 `action` 清单 + **「造一组演示数据」**一键按钮；打开后同一面板给出
   **「清空沙盘（回真实面）」**。两侧视图底部各有一块同功能面板（`order:98`）。
2. 入口是两个**动作**（`sandbox.seed` / `sandbox.clear`），走同一个动作总线 —— 沙盘**没有独立路由**。
...
1. **表格在小屏是可操作形态**：≤560px 时表格自动变**卡片**（每个 `<td>` 带 `data-label`，列名跟着值走；
   ...
   （渐变 + 一行字）。逃生门：`q-keep-table` 保持表格形态。
...
4. **键盘与屏幕阅读器**：省跳链接是第一个 Tab 站；弹层打开时给 `#q-app` 加 `inert` + `aria-labelledby`
   ...
   `main`/`banner`/`contentinfo` 地标齐备；实测可访问性树 118 个交互节点**零无名**。
5. **对比度 ≥ WCAG AA**：主题 token 按 WCAG 2.1 算过（`--accent`/`--bad` 本批上调以过 `--accent-soft` 上的 4.5），
   ...
   `docs/work/evidence/EV-193-claim-vs-impl-audit.md`（修法在外壳 `app.css`）。
...
5. **沙盘**：`demo.procurement` 第 12 步 = 供应商回签（`optional: true`）；两侧由机制生成的演示身份驱动
...
## 24 两支审计尺子的读数（判据与用法仍在主文件）

P46 实测：body 键集偏差 13→3（剩 3 条全是「多写者形状不同」）；承诺↔实现 157 = 承接 152 / 缺口 0 / 漂移 5。两把尺子都**不是门**。
