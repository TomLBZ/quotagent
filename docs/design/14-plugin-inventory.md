...
| `host/modules/gate-timeline.mjs` | **「审批等多久 / 变更单到底是谁卡着」**（domain 插件，正面回答两条 human problem：**审批人等不到** 与 **变更单扯皮**）：吃调用方给的**白名单事实载荷**（本视角 `approval/*` / `change/*` 行 + `as_of`），派生 ① 还在等的人工门（`id/kind/subject/owner/age_seconds/age_basis/consequence/next_action/blocked_by`：挂了多久 + **口径** + 卡在谁手里 + 超时策略三种后果 `remind|escalate|abort`（**没有"超时自动批准"**）+ 可复制的真 CLI 与催办路由）② 变更单时间线（`id/state/owed_by/waiting_since/basis/next_action`：状态 + **谁欠谁一个动作** + 从哪条事件起在等 + 账本事件/计数引用）。**`age_seconds` 的口径 = `as_of − approval/requested 事实 ts`，绝不取墙钟**（两个墙钟入口 `payload.now` / `config.now` 读都不读 ⇒ 同一份快照在任何"当前时间"下输出逐字节一致）；**永远不能批准**（服务面无 `approve/decide/submit` 这类方法、`meta.can_approve=false`）；空投影 → `degraded` + 有名 reason + **两列表为 0**；有界、确定性、私域零泄漏；零写面（不读账本、不写文件、不取墙钟、不联网、不调模型） | `gateTimeline` | `webui`（双方视角 `/quotagent/<view>/gates/`、`/quotagent/<view>/api/gates`、`POST /quotagent/<view>/gates/nudge`；四道页面子导航含入口） | 只改本文件 + `host/modules/webui.mjs` 的载荷装配与页面 + `tools/gate-nudge.py`（**唯一落账本者**：落 `gate/nudged`，body 恰 5 键不含理由正文）；围栏门 `host/t282-gate-timeline-gate.mjs` + 真路由门 `tools/check-gate-timeline-route.py`（`verify.sh gates`） |
