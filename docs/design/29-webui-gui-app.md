...
用户口径：「新人不看文档也要看得见一条真实流转长什么样」。本节是这一条的**机制真源**；用法见
`src/system/webui/docs/sandbox-and-demo.md`，复跑 `python3 tmp/p7-sandbox-verify.py`。
...
用户口径：「手机上真能用；新人不看文档能用；键盘全流程能走完」。真源在本节；实测证据与测量值见
`tmp/p7-a11y-out.txt` 与 `tmp/p7-report.md`（复跑：`tmp/p7-sandbox-verify.py` 之外的三宽度走查见报告内的命令）。
