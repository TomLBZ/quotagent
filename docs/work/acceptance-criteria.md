...
| AC-PLUGIN-006 | P2 | 注入式 UI 注册面：两个样板插件各注册一个只读区块并**在真页面上真出现**（两页脚本只来自受信来源）；`/api/ui/blocks` 只回执元数据；只读路由 POST ⇒ 405 + `Allow: GET`；`host/modules/webui.mjs` 里 0 次出现它们的 id/标题（webui 不懂业务） | `tools/verify.sh plugin-lifecycle` |
