...
**EV-180 批的追加**（为给新行腾预算）：主文件 §P1 的 **20 行**（`T-275`…`T-288`，第二格 `P1`、`status` 列**恰为 `done`**、evidence 列非 `—` —— 唯一例外是 `P3-UX2`（evidence `—`，但它也是 done 行，故与其余 19 行一并搬））按**原文件行序**整行逐字追加在文末（与既有各批一样是"逐批追加"，故文末不全局单调 —— 如实记录，不假装整文有序）。
**P46 批的追加**（为给新行腾预算）：主文件里第二格 `P2`、`status` 恰为 `done` 的 **`T-318`/`T-319`** 两行按**原文件行序**整行逐字追加在文末（与既有各批一样是「逐批追加」，故文末不全局单调 —— 如实记录，不假装整文有序）。
...
| P36-REGRESSION-CLEANUP | P2 | **全量回归 + `tmp/` 清理**（用户 2026-09-23 指令，只读回归、不 commit）：套件 `s1..s4` 全绿（16/16 断言）；逐门 58/59 绿 —— 红 `clean-copy`/`evolve-module`/`p0-no-node`（`g1` 内含 `qa all` 81/83）**全部既有红且逐条归因**（P16/P17 的两条「旧 UI 形态」AC 注册断言、P31/P32 改了自进化产物未重记产出）；双岗位走查：私有 data-dir 双闭环 rc=0、沙盘 `seed` 红（P21 写者闸门，独立复现）；`tmp/` **4.4 G → 183 M**（删 1045 个顶层条目 + `ac/` 下 1064 个一次性目录）；死引用/死文件普查逐条以 0 消费者为据登记（含并发写者误删 `tmp/p35-shots` 事件） | – | – | done（3 门红已登记，待裁决） | EV-192 |
