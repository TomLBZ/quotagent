...
| D-071 | 已知缺陷 | 文档门 `tools/check-docs.py` 的 `md_files()` 把 `tmp/**/*.md` 也算进扫描范围（267 个 .md 里 166 个在 tmp/），与"整树副本门"（`new_scratch` 复制/删除 `tmp/ac/<rand>/clean/**`）形成 TOCTOU：实测 16 次文档门里 2 次 `FileNotFoundError`（`check-docs.py:134/152`）→ 文档门红 → 内嵌跑它的 `AC-RUNTIME-002` 红。**未修**（改它要动 AC-RUNTIME-001「门扫描范围不变」的前提，属另一轮 + 门主确认） | `p0-no-node` 在**持续**并发整树副本下仍可能红；现在改为"连续两次红 + 原样回显 + 日志路径"，可诊断（EV-151 §三/§四） | open（本轮范围外） |
