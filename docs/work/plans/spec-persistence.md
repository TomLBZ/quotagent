...
| 6 | `docs/work/plans/config-凭据UX规格.md.txt`（43746 B，28 节；`tmp/` 有同内容副本） | §2 配置分层（项目/插件/凭据）、§3 UI 化方案（含干跑预览、0600 待办件、原子写、回滚）、§4 凭据安全（只存指针与指纹，永不回显）、§5 初始化体验 | `FR-CONFIG-001..002`（配置面）、`FR-ADMIN-006..010`（管理道）、`FR-MAIL-001..002`（凭据项之一）、`AC-CONFIG-001`；设计落点 `docs/design/21-admin-console-contract.md` + `docs/work/deployment-manual.md` | 已持久化 | `tools/verify.sh config-route`（11 条路径未提权 401 同形 / 干跑零落盘 / 保存只落 0600 / `--init` 生成 YAML / 真 config.yaml 指纹前后不变） |
...
| 13 | `tmp/config-凭据UX规格.md.txt` · `tmp/gui-交互规格.md.txt` · `tmp/ux-双方痛点与交互需求.md.txt` | 三份规格的 `tmp/` 副本（与 `docs/work/plans/` 同名文件**逐字节相同**：43746 / 50676 / 63936） | 副本无需单独落点：以 `docs/work/plans/` 里的那个为事实源（#1/#4/#6） | 已持久化（副本仅作草稿区） | 字节数对照（EV-162 §1） |
...
| G-5 | `tmp/` 与 `docs/work/plans/` 里**其余**散件（脚本、`.out`、探针） | 它们是过程产物不是规范；但**不删**（用户要求） | 本表 | 若某个过程产物后来成为规范，按同一张表补一行 |
...
# ③ 三份大规格在 tmp/ 与 docs/work/plans/ 是否逐字节相同
cmp tmp/ux-双方痛点与交互需求.md.txt docs/work/plans/ux-双方痛点与交互需求.md.txt && echo same
```
