# 证据索引（P0 mock：EV-001..EV-038）

> P1（mvp demo）阶段的证据在 [`INDEX-P1.md`](INDEX-P1.md)：EV-039 起。每份证据的原始命令输出在 `EV-<编号>-*.txt`。
...
| EV-036 | V-002 备料自检：表单 → 归一化（20 行/5 bps）→ 护栏（0 Flag）→ 比价（含税 174486.0、TCO 193803.844、18 引用）。**不是现场结论** | T-117 备料 |
| EV-037 | V-011 备料自检：QEP 构造 → 原子落盘 → 读回验签通过 → 篡改 body 后被拒；给出可复算 sha256。**不是现场结论** | T-117 备料 |
...
| EV-153 | 「审批等多久 / 变更单谁卡着」闭环：围栏门 33/33（**age 不随两个不同 `now` 入口变化** + 空投影两列表为 0 + 插件不能批准 + 每条有 basis + 4 处单点变异全红且还原字节一致）；真 HTTP 11/11（两视角 200 / 催办 POST 落 **0600** 待办件且账本零新增 / 真跑 `gate-nudge.py` 落 `gate/nudged`（body 恰 5 键、不含理由正文）且 `/api/status` 计数 **6 → 7** / 幂等 `duplicates` / 两条拒绝路径 / 私域哨兵 0 次）；`AC-GATE-001` 12/12 | AC-GATE-001 |
| EV-154 | 变更单**逐行明细**：围栏门 22/22（4 处变异全红 + 还原字节一致）+ 真路由门 9/9 + `AC-GATE-002` 8/8 | AC-GATE-002 |
