...
| EV-152 | D-071 收口：文档门扫描范围收窄为**契约文档集合**（决策 D-072；267→98、引用 6128→2798，预算 98 个文件受检与 FR↔AC 覆盖口径不变）+ 真契约文档在读窗口里消失**仍判红**（`read_md` 不跳过；仪表化拉长读窗口后删 `docs/work/roadmap.md` → exit 1 且失败行指名该文件，对照不删 → exit 0）；`AC-RUNTIME-001` 那条断言改为「**契约文档集合**不变」+ 门自报扫描数对账 + 定义文件仍在范围内 + 只断言本用例自己的 scratch（10→13 条；非空转用『跑 AC 期间移走真契约文档 → 断言红并给 diff』实测）。并发 12/12 绿（同条件旧门 11/12 红、churn 更快时 12/12 红，崩溃帧停在 `check_ids:134`） | D-071 / AC-RUNTIME-001 | P2 |
| EV-155 | FR 归档合法化：门把 `functional-requirements-archive*.md` 并入 **FR 定义集合**（`check-docs.py`：ID 完整性/预算不变、FR↔AC 覆盖扩到集合、归档 0 条 FR 行硬断言、输出 `fr_archives=[...]`；`check-fr-coverage.py`：矩阵双向全覆盖按同一集合 + 归档空读守卫）；47 条最老非 P0 行逐字搬入新建归档，主文件 32583→27645 B、归档 7754 B、handover 1015→796 B；反向验证 3 例（整仓副本上做：归档抽 1 行→`docs` 红 exit=1 且还原 sha256 一致；主文件抽 1 行→红两处（未解析 ID + AC-RUNTIME-003 孤儿）；真存在但 0 条 FR 行的归档→`docs` 与 `coverage` 双红）；12 次连续 `docs` 12/12 绿、窗口内 `docs/work/*.md` 逐字节未变 | AC-DESIGN-003 / FR↔AC 覆盖 | P2 |
