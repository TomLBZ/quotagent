...
| USREQ-005 | 「webui 的定位是**面向用户的 app**（一般用户不熟 CLI），不是一堆报告」 | `host/modules/webui.mjs`（工作台首屏 + 子导航 + `/quotagent/start/` 上手页）、`host/modules/ui-feedback.mjs` | `system/webui` | partial | 门：`webui`（8 个子视图 + `data-subnav` + 「上手」入口 + 空结果 `data-empty` 显式说明）、`ui-feedback`（反馈入口）。**缺口**：没有任何断言把「用户不需要 CLI」变成事实（现存写操作仍要把命令拿到别处执行） |
