#!/usr/bin/env python3
"""governor 门：驱动 `host/governor.mjs`（准入/背压/超时/有界重试）。
...
def main() -> int:
    ...
    proc = subprocess.run([str(ROOT / "tools" / "cordis.sh"), "run", "governor.mjs"],
    ...
    print(f"RESULT: {'PASS' if not failed else 'FAIL'}（governor 门 {len(report['checks']) - len(failed)}/{len(report['checks'])}）")
