#!/usr/bin/env python3
"""bridge-canary 门：驱动 `host/bridge-canary.mjs`（调用面分流 + 失败隔离 + 样本回灌）。
...
def main() -> int:
    ...
    proc = subprocess.run([str(ROOT / "tools" / "cordis.sh"), "run", "bridge-canary.mjs"],
    ...
    print(f"RESULT: {'PASS' if not failed else 'FAIL'}（bridge-canary 门 {len(report['checks']) - len(failed)}/{len(report['checks'])}）")
