...
- **性质类断言写完必须当场变异自证**（有界 / 定序 / 投影 / 幂等 / 只读 / 不写账本）：改一处实现 →
  门必须真红 → 还原并核对 sha256（D-057）。抓不到偷改的断言只是装饰。
  变异**放在门自己里面**（既有范式：`plugin-assets` 的 F1–F4、`plugin-requirements` 的 F1–F5、`run-once` 的 4 处、
  各插件围栅门的 4 处单点变异），自带防假变异（锚点未命中或字节没变 → 判"假变异"并失败），
  且只写在 `tmp/` 的整树副本里；独立的 ui-mutate 型变异门已按 `docs/design/29-webui-gui-app.md` §2 删除，不再新开。
