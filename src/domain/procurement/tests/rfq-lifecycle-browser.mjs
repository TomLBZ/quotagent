...
try {
  ...
  await supplier.getByRole('button', { name: 'Review & submit', exact: true }).click(); await confirm(supplier, 'submit-quote')
  await openRequest(buyer); await buyer.getByRole('button', { name: 'Compare offers', exact: true }).click(); await buyer.getByRole('button', { name: 'Award & issue order', exact: true }).waitFor()
  assert.equal(await buyer.getByRole('button', { name: 'Award & issue order', exact: true }).isEnabled(), true)
