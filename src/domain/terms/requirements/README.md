# domain/terms 需求（**本插件自己的** FR 行 + 每条的验收方式）
...
> **本文件的位置**：就是 27 §2.1 的**标准位置**（`src/domain/terms/requirements/README.md`）—— `T-318` 期间它曾在
> `docs/work/plugin-requirements-domain-terms.md`，`T-321` 用 `git mv` 归位（**内容逐字节不变**；逐条 sha256 与归位台账见映射表 §4.2 与
...
条款：条款差异下沉（供守卫的条款冲突族使用），本身不产出业务判定。
...
|---|---|---|---|
| `FR-TERMS-001` | src/quotagent/services/terms.py | `AC-TERMS-001` | `tools/verify.sh ac AC-TERMS-001` |
| `FR-TERMS-002` | src/quotagent/services/terms.py | `AC-TERMS-001` | `tools/verify.sh ac AC-TERMS-001` |

## 对外契约（provides / 依赖）

| 面 | 内容 |
|---|---|
| provides | Python 侧 `services/terms.py`。 |
| 依赖（实测 import 目标） | 无跨插件 `code/` import（27 §5.2；承载文件只 import 标准库/共享库，实测无外部 import 目标）。 |
| 门（映射表 §1 证据列里的 `ac` 简写已展开成可跑命令） | `tools/verify.sh ac AC-TERMS-001` |
...
|---|---|
| 状态（映射表 §1） | `done` |
| 承载体（映射表 §1） | `src/quotagent/services/terms.py` |
| 证据（映射表 §1） | `ac AC-TERMS-001` |
| 缺口 | 无登记缺口（映射表 §1 与该插件名下 FR 的 AC 均已注册）。 |
| 需求文档位置 | **非标准位置**：本文件 `docs/work/plugin-requirements-domain-terms.md` ⇒ 已登记在映射表 §4.2 |

## 落地状态（`code/`）

<!-- 本行由批 `EV-176` 逐插件如实登记（机检口径见 `docs/work/plans/plugin-file-map.md` §分类）。 -->

- `code:` 部分落地 —— Python 实体 1 个已在 `code/`（`terms.py`）；宿主 **ESM 入口未接** ⇒ `entry` 仍如实报 `degraded: artifact-missing`。
