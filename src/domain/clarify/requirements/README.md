# domain/clarify 需求（**本插件自己的** FR 行 + 每条的验收方式）
...
> **本文件的位置**：就是 27 §2.1 的**标准位置**（`src/domain/clarify/requirements/README.md`）—— `T-318` 期间它曾在
> `docs/work/plugin-requirements-domain-clarify.md`，`T-321` 用 `git mv` 归位（**内容逐字节不变**；逐条 sha256 与归位台账见映射表 §4.2 与
...
澄清往返：提问/广播/包版本变化时的澄清失效（不改回已发出的澄清）。
...
|---|---|---|---|
| `FR-CLARIFY-001` | src/quotagent/services/clarify.py | `AC-CLARIFY-001` | `tools/verify.sh ac AC-CLARIFY-001` |
| `FR-CLARIFY-002` | src/quotagent/services/clarify.py | `AC-CLARIFY-002` | `tools/verify.sh ac AC-CLARIFY-002` |
| `FR-CLARIFY-003` | src/quotagent/services/clarify.py | `AC-CLARIFY-003` | `tools/verify.sh ac AC-CLARIFY-003` |
...
|---|---|
| 状态（映射表 §1） | `done` |
| 承载体（映射表 §1） | `src/quotagent/services/clarify.py` |
| 证据（映射表 §1） | `ac AC-CLARIFY-001/002/003` |
| 缺口 | 无登记缺口（映射表 §1 与该插件名下 FR 的 AC 均已注册）。 |
| 需求文档位置 | **非标准位置**：本文件 `docs/work/plugin-requirements-domain-clarify.md` ⇒ 已登记在映射表 §4.2 |

## 落地状态（`code/`）

<!-- 本行由批 `EV-176` 逐插件如实登记（机检口径见 `docs/work/plans/plugin-file-map.md` §分类）。 -->

- `code:` 部分落地 —— Python 实体 1 个已在 `code/`（`clarify.py`）；宿主 **ESM 入口未接** ⇒ `entry` 仍如实报 `degraded: artifact-missing`。
