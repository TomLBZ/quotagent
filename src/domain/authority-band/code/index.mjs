/**
 * 进树插件入口：`src/domain/authority-band/`（**迁移阶段 4.1 先行 = 骨架 + wrapper，不是实体搬迁**）。
 *
 * 实体实现仍在 `host/modules/authority-band.mjs`（阶段 4.1 实体搬迁时才移进本目录 `code/`）。
 * 本文件只把实体实现的公开面**重导出**（`export *`）：`load` 真 import 本入口后，
 * `keys` 里必须出现只有实体实现才有的导出名（`REGISTERED_ROLES` / `MONEY_UNIT` / `BAND_PREFIX` …）。
 *
 * 与 `src/domain/advice/code/index.mjs` 同一形态；本文件不新增任何语义，也不写任何文件。
 */
import * as impl from '../../../../host/modules/authority-band.mjs'

export * from '../../../../host/modules/authority-band.mjs'

/** 新布局下的插件 id（与目录名一致）；服务键仍由 `provides` 决定（= `authorityBand`）。 */
