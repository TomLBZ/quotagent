...
try {
  ...
  const loginAs = async (side) => {
    ...
    const res = await fetch(`${base}/identity/login?format=json`, {
      ...
      body: new URLSearchParams({ name: `gate-t279-${side}`, side }).toString(),
    })
  ...
  // ②b 身份门槛本身也要机检（未登录拒 / 登录后按侧放行 / 越侧 403）
  const anonJson = await fetch(`${base}/contractor/api/heuristics`, { headers: { accept: 'application/json' } })
  const anonJsonBody = await anonJson.text()
  const anonHtml = await fetch(`${base}/supplier/heuristics/`,
    { headers: { accept: 'text/html' }, redirect: 'manual' })
  const anonLocation = String(anonHtml.headers.get('location') ?? '')
  const crossSide = await fetch(`${base}/supplier/heuristics/`,
    { headers: { cookie: await loginAs('contractor') } })
  const crossBody = await crossSide.text()
  check('17b 身份门槛（P3）：未登录取业务路由 ⇒ API 401 `identity-required` + `next`、浏览器 303 回 `/identity/?next=…`；'
  ...
  && anonHtml.status === 303 && anonLocation.includes('/t279/identity/?next=')
  && crossSide.status === 403 && crossBody.includes('side-mismatch')
  && (await get('/contractor/heuristics/')).status === 200,
