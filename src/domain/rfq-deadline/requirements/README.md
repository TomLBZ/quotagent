...
> **本文件的位置**：就是 27 §2.1 的**标准位置**（`src/domain/rfq-deadline/requirements/README.md`）—— `T-318` 期间它曾在
> `docs/work/plugin-requirements-domain-rfq-deadline.md`，`T-321` 用 `git mv` 归位（**内容逐字节不变**；逐条 sha256 与归位台账见映射表 §4.2 与
...
- `code:` **待实现** —— 实现对**已存在**于 `host/modules/`（`rfq-deadline.mjs`），本批未给它做入口；`entry` 仍如实报 `degraded: artifact-missing`；**不新造功能**。
