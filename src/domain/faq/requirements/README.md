# domain/faq 需求（**本插件自己的** FR 行 + 每条的验收方式）
...
> **本文件的位置**：就是 27 §2.1 的**标准位置**（`src/domain/faq/requirements/README.md`）—— `T-318` 期间它曾在
> `docs/work/plugin-requirements-domain-faq.md`，`T-321` 用 `git mv` 归位（**内容逐字节不变**；逐条 sha256 与归位台账见映射表 §4.2 与
...
| 面 | 内容 |
|---|---|
| provides | Python 侧 `services/faq.py`（`FaqService.reuse`）+ 门 `tools/check-faq.py`。 |
| 依赖（实测 import 目标） | `src/quotagent/services/faq.py` → `..kernel.canon`、`..kernel.ledger`、`.approval`；`tools/check-faq.py` → `quotagent.qa`、`quotagent.qa.registry`（27 §5.1/§5.2：只 import 内核面或其它插件的公开服务面，不 import 别的插件实现文件） |
| 门（映射表 §1 证据列里的 `ac` 简写已展开成可跑命令） | `tools/verify.sh faq` |
...
|---|---|
| 状态（映射表 §1） | `done` |
| 承载体（映射表 §1） | `src/quotagent/services/faq.py`、`tools/check-faq.py` |
| 证据（映射表 §1） | `faq`(AC-FAQ-001 20/20) |
| 缺口 | 无登记缺口（映射表 §1 与该插件名下 FR 的 AC 均已注册）。 |
| 需求文档位置 | **非标准位置**：本文件 `docs/work/plugin-requirements-domain-faq.md` ⇒ 已登记在映射表 §4.2 |

## 落地状态（`code/`）

<!-- 本行由批 `EV-176` 逐插件如实登记（机检口径见 `docs/work/plans/plugin-file-map.md` §分类）。 -->

- `code:` 部分落地 —— Python 实体 1 个已在 `code/`（`faq.py`）；宿主 **ESM 入口未接** ⇒ `entry` 仍如实报 `degraded: artifact-missing`。
