/**
 * 进树插件入口：`src/system/ops-view/`（**本批 EV-177 的真实承载**）。
 *
 * 做什么：把**已随实体搬迁落进本目录 `code/`** 的那一份实现（`./ops-view.mjs`）的公开面**重导出**
 * （`export *`，名字绑定是活的），并把它自述的 `inject`/`provides`/`Config`/`usedServices`
 * 原样透给宿主。**不新造功能**：本文件没有一行业务语义，也没有任何写面。
 *
 * 本插件的实体是**自进化产出**（`docs/work/evolution-log.json` 里 `name=ops-view` 那条，
 * `artifact_path` 就指 `code/ops-view.mjs`）；搬迁**字节守恒**，`tools/verify.sh evolve-module` 逐字节比对。
 *
 * `plugin.json` 的 `entry` 指到本文件 ⇒ `tools/plugin.sh status system/ops-view` 从
 ...
 */
import * as impl from './ops-view.mjs'

export * from './ops-view.mjs'
