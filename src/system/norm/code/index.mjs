/**
 * 进树插件入口：`src/system/norm/`（**本批 EV-176 的真实承载**）。
 *
 * 做什么：把**已随实体搬迁落进本目录 `code/`** 的那一份实现（`./norm.mjs`）的公开面**重导出**
 * （`export *`，名字绑定是活的），并把它自述的 `inject`/`provides`/`Config`/`usedServices`
 * 原样透给宿主。**不新造功能**：本文件没有一行业务语义，也没有任何写面。
 *
 * `plugin.json` 的 `entry` 指到本文件 ⇒ `tools/plugin.sh status system/norm` 从
 ...
 */
import * as impl from './norm.mjs'

export * from './norm.mjs'
