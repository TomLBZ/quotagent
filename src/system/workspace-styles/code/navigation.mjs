const groups=[
 {id:'quotation',label:'Quotation work',icon:'file',ids:['workspace','rfqs','quotes','orders','messages','team']},
...
export function groupNavigation(items){
 ...
 const primary=take(['agent','workroom','approvals']),utilities=take(['notifications','help','settings']),sections=groups.map(group=>({...group,items:take(group.ids)})).filter(group=>group.items.length)
