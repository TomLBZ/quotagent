...
而且**任何情况下凭据值都不出现在账本/响应体/日志里**。七组断言（每条都能被证伪）：
  ...
  F **配置键能经既有保存路径落盘**（本批要求 ②，路径**不变**）：`mail.smtp.*` / `mail.imap.*` 已登记在
    `src/system/config/code/config-keys.mjs` + `src/system/config/code/schema.mjs`；宿主干跑（`previewPatch`）受理；0600 待处理项 →
...
try:
    ...
    schema_text = (ROOT / "src" / "system" / "config" / "code" / "schema.mjs").read_text(encoding="utf-8")
    ...
    check("F1 配置键登记（跨语言单一真源）：`mail.smtp.*` / `mail.imap.*` 都在 "
          "`src/system/config/code/config-keys.mjs`（类型 + 默认层）与 `src/system/config/code/schema.mjs`（可改性/人工专属）里 —— "
