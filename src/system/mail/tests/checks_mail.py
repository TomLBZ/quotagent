...
from quotagent.kernel.canon import canonical_json, is_hash
...
from quotagent.services import mail as mail_module
from quotagent.services.mail import (HeaderInjectionRejected, MailError, MailService, MailTransportUnavailable,
                             UnsupportedAttachment, UnknownMessage)
from quotagent.qa.registry import REGISTRY, Assertion, register
...
def _impl_source(module: object) -> tuple:
    ...
    （搬前实测：`__file__` 仍是旧路径 ⇒ 扫到的是重导，断言变空）。
    """
...
def _module_source() -> tuple:
    ...
    return _impl_source(mail_module)
