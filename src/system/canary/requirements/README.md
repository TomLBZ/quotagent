# system/canary 需求（**本插件自己的** FR 行 + 每条的验收方式）
...
> **本文件的位置**：就是 27 §2.1 的**标准位置**（`src/system/canary/requirements/README.md`）—— `T-318` 期间它曾在
> `docs/work/plugin-requirements-system-canary.md`，`T-321` 用 `git mv` 归位（**内容逐字节不变**；逐条 sha256 与归位台账见映射表 §4.2 与
...
自进化产物的真实路由分流与自动回滚判定（进/升需人工引用，回滚自动）。
...
|---|---|---|---|
| `FR-EVOLVE-005` | host/modules/canary.mjs | `AC-EVOLVE-003` | `tools/verify.sh canary` · `tools/verify.sh bridge-canary`（关联 AC 尚未在 `qa` 注册 ⇒ 现由本插件的门围栏，见「现状与缺口」） |

## 对外契约（provides / 依赖）

| 面 | 内容 |
|---|---|
| provides | `canary`（`host/modules/canary.mjs`）、`canary-dispatch`（`host/lib/canary-dispatch.mjs`）、`bridge-canary`（`host/modules/bridge-canary.mjs`）。 |
| 依赖（实测 import 目标） | `host/modules/canary.mjs` → `../lib/std-schema.mjs`；`host/modules/bridge-canary.mjs` → `../lib/std-schema.mjs`（27 §5.1/§5.2：只 import 内核面或其它插件的公开服务面，不 import 别的插件实现文件） |
| 门（映射表 §1 证据列里的 `ac` 简写已展开成可跑命令） | `tools/verify.sh canary` · `tools/verify.sh bridge-canary` · `tools/verify.sh canary-route` |
...
|---|---|
| 状态（映射表 §1） | `done` |
| 承载体（映射表 §1） | `host/modules/{canary,bridge-canary}.mjs`、`host/lib/canary-dispatch.mjs` |
