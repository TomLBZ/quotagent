...
# 纪律：断言**一条不删、一条不放松** —— 业务路由仍逐条要求 200/202，只是带上本侧 cookie。
# ---------------------------------------------------------------------------
...
def login_as(side: str) -> dict:
    ...
    if side not in COOKIES:
        payload = urllib.parse.urlencode({"name": f"gate-ui-feedback-{side}", "side": side}).encode("utf-8")
...
def main() -> int:  # noqa: C901
    ...
    try:
        ...
        anon_json_code, anon_json_body, _h = raw_request_full(
            f"{base}/supplier/api/events", headers={"Accept": "application/json"})
        anon_html_code, _b, anon_html_headers = raw_request_full(
            f"{base}/contractor/feedback", headers={"Accept": "text/html"}, follow_redirects=False)
        cookie_contractor = login_as("contractor")
        cookie_supplier = login_as("supplier")     # noqa: F841（两侧各登录一次）
        same_side = raw_request_full(f"{base}/contractor/feedback", headers=cookie_contractor)
        cross_side = raw_request_full(f"{base}/supplier/feedback", headers=cookie_contractor)
        cross_json = raw_request_full(f"{base}/supplier/api/events", headers=cookie_contractor)
