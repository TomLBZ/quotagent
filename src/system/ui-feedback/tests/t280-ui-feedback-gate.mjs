...
// 实体已随批 `EV-178` 搬进插件 `code/`（旧路径 `host/modules/ui-feedback.mjs` 只剩**薄重导**）：
// 本门读/变异的是**实体那一份**（否则会静默判绿：变异打在转发文件上不改变行为）。
const TARGET = join(HERE, '..', 'src', 'system', 'ui-feedback', 'code', 'ui-feedback.mjs')
...
try {
  ...
  const loginAs = async (side) => {
    ...
    const res = await fetch(`${base}/identity/login?format=json`, {
      ...
      body: new URLSearchParams({ name: `gate-t280-${side}`, side }).toString(),
    })
  ...
  const anonJson = await fetch(`${base}/supplier/api/events`, { headers: { accept: 'application/json' } })
  const anonJsonBody = await anonJson.text()
  const anonHtml = await fetch(`${base}/contractor/feedback`,
    { headers: { accept: 'text/html' }, redirect: 'manual' })
  const anonLocation = String(anonHtml.headers.get('location') ?? '')
  const crossSide = await fetch(`${base}/supplier/feedback`,
    { headers: { cookie: await loginAs('contractor') } })
  const crossBody = await crossSide.text()
  check('10b 身份门槛（P3）：未登录取业务路由 ⇒ API 401 `identity-required` + `next`、浏览器 303 回 `/identity/?next=…`；'
  ...
  && anonHtml.status === 303 && anonLocation.includes('/t280/identity/?next=')
  && crossSide.status === 403 && crossBody.includes('side-mismatch')
  && (await get('/contractor/feedback')).status === 200,
  ...
  const post = async (path, body) => {
    const res = await fetch(`${base}${path}`, { method: 'POST', body, headers: await cookieFor(path) })
    const text = await res.text()
    let json = null
