...
import {arithmeticInstruction} from '../../agent-runtime/code/product-policy.mjs'
...
export function createWorkflows(ctx,{memory,notify=async()=>{}}){
  const locks=new Map(),jobs=new Map(),controllers=new Map(),controls=new Map();let disposed=false
  ...
  const serial=(id,operation)=>{
    const task=(locks.get(id)||Promise.resolve()).catch(()=>{}).then(operation)
    locks.set(id,task);task.finally(()=>{if(locks.get(id)===task)locks.delete(id)}).catch(()=>{});return task
  }
  const trace=(user,id,event,detail={},signal)=>serial(id,async()=>{
    if(signal&&(disposed||signal.aborted||raw(user,id).status==='cancelled'))return null
    const entry={id:randomUUID(),runId:id,event,at:now(),...detail};await ctx.store.put(user.id,'workflow-traces',entry,{actor:detail.agentId?`agent:${detail.agentId}`:['created','paused','resumed','cancelled','human-answered','human-feedback'].includes(event)?`human:${user.id}`:`workflow:${id}`,event:`workflows/${event}`});return entry
  })
  const update=(user,id,change,event='state-changed',signal,allowedStatuses=['running'])=>serial(id,async()=>{
    const current=raw(user,id)
    // Check the active execution lease when the queued write commits, not when it is scheduled.
    if(signal&&(disposed||signal.aborted||!allowedStatuses.includes(current.status)))return null
    const patch=typeof change==='function'?change(current):change;if(!patch)return null
    const next={...current,...patch,revision:current.revision+1,updatedAt:now()};await ctx.store.put(user.id,'workflow-runs',next,{actor:`workflow:${id}`,event:`workflows/${event}`});return next
  })
  ...
  async function plan(user,id,signal){
    ...
    await trace(user,id,'plan-created',{agentId:'planner',summary:next.summary,details:next.steps.map(({id,title,role,task,dependsOn})=>({id,title,role,task,dependsOn}))},signal)
    if(run.checkpoint&&await checkpoint(user,id,'awaiting-plan',{kind:'plan',text:'Review the plan. Add any priorities or constraints, then approve it to start the specialist agents.'},signal)){if(!signal.aborted)await notify(user,{type:'question',title:'Your agent plan is ready',body:next.title,link:{view:'workroom',runId:id},sourceId:id,dedupeKey:`${id}:plan`})}
  }
    ...
    for(let turn=0;turn<6;turn++){
      ...
      for(const call of response.tool_calls){
        ...
        try{args=JSON.parse(call.function.arguments||'{}');if(call.function.name==='ask_workflow_human'){
          ...
          if(!await update(user,id,current=>{
            ...
            return {status:'waiting-input',questions,question:questions.find(entry=>entry.status==='pending'),steps:current.steps.map(candidate=>candidate.id===stepId?{...candidate,status:'waiting-input',messages:wire,question}:candidate.status==='running'?{...candidate,status:'queued'}:candidate)}
          },'waiting-input',signal,['running','waiting-input']))return
          await trace(user,id,'human-question',{stepId,agentId:step.agentId,summary:question,details:{reason:args.reason}},signal)
          if(!signal.aborted)await notify(user,{type:'question',title:`${step.role} needs your input`,body:question,link:{view:'workroom',runId:id},sourceId:id,dedupeKey:`${id}:${stepId}:${step.attempt}:question`});return
        }
        ...
        await trace(user,id,'tool-result',{stepId,agentId:step.agentId,summary:`${call.function.name}: ${result?.error || 'completed'}`,details:{tool:call.function.name,arguments:args,result}},signal)
  ...
  async function execute(user,id,signal){
    try{
      ...
      while(running(user,id,signal)){
        ...
        if(!ready.length){await checkpoint(user,id,'paused',{kind:'recovery',text:'A task was interrupted. Resume to retry unfinished work.'},signal);return}
        await Promise.all(ready.map(async step=>{try{await worker(user,id,step.id,signal)}catch(error){if(running(user,id,signal)){await stepUpdate(user,id,step.id,{status:'failed',error:error.message},signal);throw error}}}))
      }
      if(!running(user,id,signal))return
      const run=raw(user,id)
  ...
  async function applyControl(user,id,{action,text}={}){
    ...
    if(action==='cancel'){controllers.get(id)?.abort();await update(user,id,current=>({status:'cancelled',stopReason:'human-stop',question:null,cancelledAt:now(),steps:current.steps.map(step=>step.status!=='completed'?{...step,status:'cancelled'}:step)}));await trace(user,id,'cancelled',{summary:'The user cancelled this run.'})}
      ...
      await trace(user,id,action==='answer'?'human-answered':action==='feedback'?'human-feedback':'resumed',{summary:text||'The user approved the plan or resumed unfinished tasks.'})
    }else throw failure('Choose pause, resume, cancel, answer or feedback.')
    return get(user,id)
  }
  function control(user,id,input={}){
    // A job may finish synchronously when aborted. Block its finally/relaunch path until the human transition is durable.
