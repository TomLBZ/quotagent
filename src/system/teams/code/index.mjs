...
export function apply(ctx){
  const teams=createTeams({store:ctx.store,accounts:ctx.accounts,notifications:()=>ctx.get('notifications'),procurement:()=>ctx.get('procurement')})
  ...
  route('GET','/teams',({user})=>teams.state(user))
