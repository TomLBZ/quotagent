...
from quotagent.qa.registry import Assertion, ACCheck, run_check, register
...
def ac_runtime_001() -> list[Assertion]:
    ...
    # 阶段 5 第二小片（EV-174）：服务实体搬进 `src/<层>/<插件>/code/`，旧路径留**薄重导** ⇒
    # 扫描面**两边都覆盖**（实体 + 旧路径那一份），否则"服务不依赖宿主运行时"这条会在只有 20 行
    # 薄重导的文件上判绿（= 断言静默变空）。
    services_entity = sorted(p for sub in ("system", "domain")
                             for p in (root / "src" / sub).glob("*/code/*.py")
                             if p.parent.parent.name != "kernel")
    services_files = sorted((root / "src/quotagent/services").glob("*.py"))
    p0_files = kernel_files + services_entity + services_files + [root / "src/quotagent/paths.py"]
    offenders = []
    ...
    out.append(Assertion("P0 内核与服务不依赖宿主运行时（无 node/cordis 引用；宿主机能力属 P1 的桥接 AC）",
                         not offenders and len(kernel_entity) >= 8 and len(services_entity) >= 8,
                         "; ".join(offenders[:4]) or
                         f"检查 {len(p0_files)} 个文件（内核 {len(kernel_files)}：实体 {len(kernel_entity)} + 薄重导 "
                         f"{len(kernel_files) - len(kernel_entity)}；服务实体 {len(services_entity)} + 旧路径 "
                         f"{len(services_files)}）；扫描面非空转（内核实体 ≥8 且服务实体 ≥8 个 .py）"))
