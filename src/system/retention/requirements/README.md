...
- `code:` **已落地** —— 实体 `code/retention-view.mjs`（本批随宿主模块搬迁进 `code/`，**字节守恒**，`evolve-module` 逐字节校验）+
  入口 `code/index.mjs`（只把实体公开面**重导出**：`export *` 的绑定是活的，无业务语义、无写面）。
- `provides:` `retentionView`（实体自述的真实服务键；占位键已改写）。
- 实测：`tools/plugin.sh status system/retention` ⇒ `valid:true`、`reason:null`；`load` 真进口（`effects` 非 0）、`unload` 后 `effects_after:0`。
