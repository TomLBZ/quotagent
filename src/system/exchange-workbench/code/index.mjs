...
export async function apply(ctx,config={}){
 ...
 const receive=async(target,packageValue,{user}={})=>{
  if(Array.isArray(packageValue)){if(!packageValue.length)fail('The delivery batch is empty.');let result;const released=[];for(const item of packageValue){result=await receive(target,item,{user});released.push(...(result.released||[]))}return {...result,released}}
  ...
  const receipts=ctx.store.deliveryState(target).outbox.filter(row=>row.control&&row.to===packageValue.envelope.sender.realm&&row.channel===packageValue.channel).map(row=>({schema:'quotagent/qep-package/v1',channel:row.channel,envelope:row.envelope}));
 ...
 const list=user=>{
  ...
  return{realm:target,canManageRoutes:user.id===target,outbox:state.outbox.filter(row=>!row.control).map(thin).reverse(),inbox:state.inbox.filter(row=>!row.control||row.status==='held').map(thin).reverse(),peers:ownPeers(user).map(row=>({...row,paired:row.mode==='local'||ctx.settings.view(owner(row.ownerId),row.configurationId).configuredSecrets.includes('pairingSecret')})),localAccounts:ctx.accounts.list().filter(row=>row.role!=='admin'&&row.id!==target).map(({id,name,role})=>({id,name,role})),capabilities:ctx.store.capabilities,health:ctx.store.health()[target]||{healthy:true,eventCount:0},receiverPath:`${ctx.web.prefix||'/quotagent'}/api/exchange/receive/${encodeURIComponent(target)}`}
 }
