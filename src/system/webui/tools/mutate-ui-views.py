...
**位置（本批迁移）**：实体在 `src/system/webui/tools/mutate-ui-views.py`；旧位置 `tools/mutate-ui-views.py` 只剩**薄转发**
...
MUTATIONS = [
    dict(name='M1 写入器上限 5→50（有界被破坏）', path='src/system/webui/tools/refresh-ui-snapshots.py',
         old='RECENT_LIMIT = 5', new='RECENT_LIMIT = 50', gate=['ui-seed']),
    dict(name='M2 写入器倒序→正序（最新在后）', path='src/system/webui/tools/refresh-ui-snapshots.py',
         old='return list(reversed(picked[-RECENT_LIMIT:]))', new='return picked[:RECENT_LIMIT]',
         gate=['ui-seed']),
    dict(name='M3 写入器原样透传（正文/私域随行出去）', path='src/system/webui/tools/refresh-ui-snapshots.py',
