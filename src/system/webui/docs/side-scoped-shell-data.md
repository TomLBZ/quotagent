...
### 2.1 「越侧」用哪个码：**一处权威（P52 定）**

两个码不是"同一个意思的两种写法"，它们各管**一个面**；曾经因为码不唯一，按码判定的运维/脚本会猜错
（P51 §6-F6 登记）。P52 把口径定死如下——**只有这两条，不得再产生第三个越侧码，也不得在两个面之间混用**：

| 面 | 码 | HTTP | 谁判 | 什么时候出现 | 账本/文件 |
|---|---|---|---|---|---|
| **读路由 / 数据面**（`/app/<侧>/**`、`/api/ui/panels|object` 的 `view`、面板行） | `side-mismatch` | **403** | `identity.mjs#gateBusinessRoute`（外壳路由）与 `webui.mjs#sideVerdict`（数据路由）——**同一份判据** | 拿着**一侧的会话**去读/打开**另一侧**的页面或数据（或该侧不复存在） | 零新增、零改动 |
| **动作面**（动作的服务端一半） | `cross-side-action` | **400** | 各插件动作自己的 `sideGuard` / 声明了这条判据的服务端一半（见 §2 表） | 请求体/动作声明的 `view` ≠ **会话侧**，且这个动作**声明了**这条判据（在**任何写之前**判） | 零新增、零改动 |
| **未登录** | `identity-required` | 401（HTML ⇒ 303 去登录） | 同上前两处 | 没有会话 cookie | 零新增 |

**判据一句话**：`side-mismatch` 管「**这条数据 / 这一页不归你这侧**」，`cross-side-action` 管
「**你要在另一侧发起一个动作**」。两者都在**写之前**、都保证账本零新增；只读动作（如
`authority.check`）不声明动作面判据 ⇒ **不产生任何一个码**。

**P52 实测（真跑，读数在 `tmp/p52-shots/`）**：承包商会话请求 `/app/supplier/` ⇒ 403 `side-mismatch`；
`/api/ui/panels?view=supplier` ⇒ 403 `side-mismatch`；同一个会话把 `view: 'supplier'` 发给
`gate.grant` / `award.confirm` / `authority.bands.set` ⇒ 400 `cross-side-action`（两条路各自零新增，
两侧账本 sha256 前后一致）。**同一个越侧意图会先撞读路由的 403**（页面打不开就没得点）——动作面那条是
给手搓请求/别的客户端准备的。
