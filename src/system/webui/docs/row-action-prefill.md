...
## 4. 邻居规矩：**行数组的读数**（坏行逐条计数、好行照列）

（P27 复现、P30 收口；读数与逐条登记见 `tmp/p30-shots/REPORT.md`）面板对**外部给的行数组**（投递信封
`spec.items`、账本 `body.lines`、工具 JSON 的 `evidence.*`、收件箱 `quote.items`、意向信封数组）**不许裸读
元素属性**：`(body.lines ?? []).map((line) => line.item_id)` 只要数组里混进**一条** `null`/字符串，
读属性就抛 `TypeError`；而外壳对 `panel.data()` 抛错的处理是**整块面板判 `data-failed`**
（`app-shell.mjs`；页面其余部分照常，**这一块整块没了**）。
...
4. 源本身**不是数组** ⇒ 那是「读不出来」，**不是「零行」**（两者必须长得不一样，别拿空表顶替）。

自查（**可重跑**）：`python3 tmp/p27-shots/audit-row-shapes.py --verbose` 逐文件列出裸读处数。
