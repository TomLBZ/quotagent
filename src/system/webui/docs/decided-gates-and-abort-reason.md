...
## 5.5 「被谁消费」：批准人是不是**署名的人**（P48）

四问答的是**门自己**；P0 的缺陷出在门与下游的关系上（写者自己开单自己批）。面板因此多一列
**「被谁消费（下游署名 vs 谁批的）」**（从账本算，界面不推）：找不到引用了这扇门的下游事实（同视图、
`body.approval_id` 同值、非 `approval/*`）⇒ 如实写「**批了但没被用上**」，不假装它撑住了什么；署名取那条
**账本行的 `actor`**，与 `decided_by` 比：不同 ⇒「**谁批的 ≠ 谁签的**」+ 行号，相同 ⇒「**批准人就是署名人**」
（自签自批：旧行，或显式开了开关）。计数 `approver_differs` / `not_consumed` 一并给。
