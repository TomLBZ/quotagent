...
4. **中断与恢复**（P23 真跑读数见 §5.2）：进程被杀之后，`jobs.json` 里那一条还写着 `running`；下一次读
   ...
   **收尾对账（P25 ③）**：重试是**新的一批**（新 `job.id`），旧记录**不删**；读 `/api/ui/jobs` 时把它与
   **后来的批次**对账（已拿到结论的进 `superseded`）⇒ 全做完后 `unfinished=0`、提示条**自然消失**；
   非 0 时仍摆，且可「忽略这条（不再提示）」（只收起、不删数据）。见 `p25-truth-and-short-viewport.md` §3。
...
| ⑧ 界面侧 | 提交期间轮询进度「正在逐份落账 N/M」；重启后摆「上一批没有跑完：…还没做的 26 份」+「只重试剩下的 26 份」（P25 起只在**真有未完成**时摆、可忽略） |
