...
**已决定门的四问**（谁 / 何时 / 为什么 / 依据哪一行）与**终止理由进账本**见 `decided-gates-and-abort-reason.md`。
...
5. **主管/审批人视角（P41 走查 → P42/P43 收口）**：从一条 PO 回溯到报价与门是**五段**（末段 = 人工门：
   结论 / 谁批的 / 什么时候 / 为什么）；门对象页给出请求人·请求时刻·点名审批人·谁批的·意见。**终止（abort）
   的理由正文逐字进账本**（`approval/aborted.comment`，ADR-0023）⇒ 已决定的门「谁决定的 / 什么时候 /
   为什么 / 依据哪一行」一屏可答、旧行按缺省如实呈现；仍做不到的见 `decided-gates-and-abort-reason.md`；
   **P43 已闭合**队列无金额列、卡头按侧算、拒绝文案露 CLI 旗标三条（见 `people-and-roles.md` §10）。
