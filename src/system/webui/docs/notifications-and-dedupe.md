...
## 8. **只读探测不发通知**（P49：未读不被界面自发的读取污染）

**症状**（P47 §4.5 实测）：通知中心里混进**界面自己发的**只读动作。例：`export.columns` 的**只读模式**
（`report_id` 留空 = 「把'我这套列选择'读回来」，由 `assets/app.js#hydrateExportPrefs` 在页面加载时按需发）
每次都会在动作流水里留一条：登录时一条 `ok`、**未登录时一条 `identity-required`（`bad`）** —— 用户什么都
没点，未读数却被读操作顶上去（修前实测：打开一个对象页 ⇒ 未读 13 → 14，并新增
`act-1790161166661-export.columns`）。

**判据（两层，一层客户端一层机制）**：

1. **客户端**（`assets/app.js#hydrateExportPrefs`）：**未登录就不发这一次探测** —— 列选择是**按身份**存的
   （0600），没有身份时这次调用只可能被如实拒，而它是界面自发的、不是用户点的。用户**自己**点保存时照旧发
   （照旧可被拒）。
2. **机制**（`app-shell.mjs#runActionInner` + `ui-surface.mjs` 的动作契约）：插件可以在**这次派发的回执**上
   声明 `readonly: true`（「这一次是只读的」，由插件自己说；只读分支被判拒时也仍是只读的）。三条同时成立才
   **不记进动作流水**（动作流水是通知中心里的一类条目）：
   ① 插件声明了 `readonly: true`；② **真的没跑任何唯一写者**（`writer.verdict === 'no-writer-run'`）；
   ③ 账本新增为 0。⇒ 它**不可能**遮住「写发生了」或「写者回执与响应不一致」的差异（那两类不满足②），
   跳过的这一条照旧在宿主日志里留一行（不静默）。**保存类派发不要声明它。**

**读数（同一份数据、同一段序列，修前 = `git archive HEAD` 起的 8586 / 修后 = 工作树 8587）**：

| 序列 | 修前（8586） | 修后（8587） |
|---|---|---|
| 已登录：打开对象页两次（每次都发只读探测） | 未读 **13 → 14**（新增 `act-…-export.columns`），再打开继续加 | 未读 **9 → 9 → 9**，`export.columns` 条目 **0** 条；宿主日志 6 行「只读探测不进动作流水」 |
| 未登录：打开对象页两次 | 通知中心里 **5 条** `导出：这里有哪几列（个人偏好） → identity-required`（其中 2 条来自这两次加载） | **0 条**（客户端判定未登录 ⇒ 这次探测**根本没发**） |
| 真·保存列选择（用户点了保存） | 照旧留痕 | 照旧留痕（`readonly` 只在只读分支声明） |

**同时修掉的一条**（同批 P47 §4.5）：**弹层里只显示"本次动作的回执"**。修前 `openModal` 会把浮层上
**上一条动作**的提示（6–12 s 内还没消失）整批搬进新开的弹层 ⇒ 打开一个表单，正文最上面挂着上一条动作的
「已受理…」。现在浮层那份**降到遮罩之下**（`body.q-modal-open .q-toasts{z-index:20}` < `#q-modal` 的 40），
只有**本次**动作的提示进弹层正文（`toast()` 写的 `[data-toast-host]`）。读数：跑一次动作后立刻重开表单 ——
修前弹层内 `.q-toasts-inline .q-toast` **1 条**（正是上一条回执）、修后 **0 条**（那条仍在浮层里，z-index 20）。
截图 `tmp/p49-shots/{10,11}-*.png`；修前/修后原始读数见 `tmp/p49-shots/REPORT.md` §3。
