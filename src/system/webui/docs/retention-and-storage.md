...
| **同一进程的"真实面 / 沙盘面"同时写** | 锁面 = **本会话有效数据目录**（`effective().dir`）：**同一面内**写者互斥、**跨面**互不排队（两个面两份账本） | 真实面 6 条并发 + 沙盘面 6 条并发同时跑：12/12 成功、两侧账本无重复 `seq`/无断链、`timeouts` 增量 0、跨面持锁 0 干扰（`tmp/p38-shots/gate-faces-probe.py`，29/29） |
