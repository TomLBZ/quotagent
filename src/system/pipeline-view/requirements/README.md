...
|---|---|---|---|
| `FR-UX-005` | tools/refresh-ui-snapshots.py、host/modules/pipeline-view.mjs | `AC-PIPELINE-001` | `tools/verify.sh pipeline-view` · `tools/verify.sh pipeline-route` |
...
| 门（映射表 §1 证据列里的 `ac` 简写已展开成可跑命令） | `tools/verify.sh pipeline-view` · `tools/verify.sh pipeline-route`（写入器的真输出由 `pipeline-view` 门第 13 条在**真账本**上与手算表逐字段对账；旧口径的 UI 快照/种子类 AC 已按 `docs/design/29-webui-gui-app.md` §2 删除，不留副本） |
...
| 证据（映射表 §1） | `pipeline-view`(7/7) · `pipeline-route` · `ac AC-PIPELINE-001` |
