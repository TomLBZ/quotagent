...
try {
  ...
  const callsBeforeHistory = fixture.calls.length, removedHistory = ctx.connections.detail(buyer, legacy.id)
  assert.equal(removedHistory.connection.deleted, true); assert.equal(removedHistory.calls[0].result.structuredContent.total, 25, 'Removed connection retains its real reviewed result')
  assert.ok(!ctx.connections.list(buyer).some(row => row.id === legacy.id), 'Normal list omits removed connections')
  assert.throws(() => ctx.connections.detail(supplier, legacy.id), /not found/, 'Removed history remains account-owned')
  assert.throws(() => ctx.connections.get(buyer, legacy.id), /not found/)
  await assert.rejects(() => ctx.connections.discover(buyer, legacy.id), /not found/)
  await assert.rejects(async () => ctx.connections.run(buyer, legacy.id, { operation: 'tool', input: { name: 'landed_cost', arguments: { quantity: 9, unitPrice: 9 } } }), /not found/)
  assert.equal(fixture.calls.length, callsBeforeHistory, 'Reading removed history never calls the remote service')
  ...
  const restoredHistory = ctx.connections.detail(buyer, a2a.id)
  assert.equal(restoredHistory.connection.deleted, true); assert.equal(restoredHistory.tasks.length, 2)
  assert.match(restoredHistory.tasks.find(row => row.status === 'TASK_STATE_COMPLETED').task.artifacts[0].parts[0].text, /120 USD/, 'Removed A2A history and artifacts survive restart')
