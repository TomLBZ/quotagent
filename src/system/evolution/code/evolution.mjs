...
// 实体搬迁（迁移阶段 5，本批 `EV-178`）：`host/lib/evolution.mjs` → `src/system/evolution/code/evolution.mjs`。
// 仓库根由 **4 层**上溯推出（`code/` → `evolution/` → `system/` → `src/` → 仓库根）；旧路径
// `host/lib/evolution.mjs` 只剩**薄重导**（`export *`，名字绑定是活的）。
 ...
 *
 * 为什么本文件必须**显式解析**：实体已从 `host/lib/` 搬进 `src/system/evolution/code/`，而 Node 对裸名
 * `cordis` 的上溯只看 `node_modules` 目录链 —— 从 `src/` 下**上溯不到** `host/node_modules/`（实测
 * `ERR_MODULE_NOT_FOUND`）。解析顺序与 `src/system/runtime/code/plugin-registry.mjs` 的 `loadCordis`
 * **同一套**（不新增第二套解析规则）；依赖声明在 `src/system/evolution/plugin.json` 的 `dependencies`。
 * 唯一的语义差别：本模块的解析发生在**模块求值期**（顶层 await），拿到的仍是同一份 `cordis` 单例。
 */
...
const { Context, EventsService } = await loadCordis()
