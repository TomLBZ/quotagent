# system/idempotency-guard 需求（**本插件自己的** FR 行 + 每条的验收方式）
...
> **本文件的位置**：就是 27 §2.1 的**标准位置**（`src/system/idempotency-guard/requirements/README.md`）—— `T-318` 期间它曾在
> `docs/work/plugin-requirements-system-idempotency-guard.md`，`T-321` 用 `git mv` 归位（**内容逐字节不变**；逐条 sha256 与归位台账见映射表 §4.2 与
...
运行期请求幂等判重：同请求不重复打下游，失败结果是 replay 而非复用成成功。
...
|---|---|---|---|
| `FR-RUNTIME-009` | host/modules/idempotency-guard.mjs | `AC-RUNTIME-009` | `tools/verify.sh idempotency-guard` · `tools/verify.sh idem-route`（关联 AC 尚未在 `qa` 注册 ⇒ 现由本插件的门围栏，见「现状与缺口」） |

## 对外契约（provides / 依赖）

| 面 | 内容 |
|---|---|
| provides | `idempotency`（`host/modules/idempotency-guard.mjs`）。 |
| 依赖（实测 import 目标） | `host/modules/idempotency-guard.mjs` → `../lib/std-schema.mjs`（27 §5.1/§5.2：只 import 内核面或其它插件的公开服务面，不 import 别的插件实现文件） |
| 门（映射表 §1 证据列里的 `ac` 简写已展开成可跑命令） | `tools/verify.sh idempotency-guard` · `tools/verify.sh idem-route` |
...
|---|---|
| 状态（映射表 §1） | `done` |
| 承载体（映射表 §1） | `host/modules/idempotency-guard.mjs` |
...
- `code:` **已落地** —— 实体 `code/idempotency-guard.mjs`（本批随宿主模块搬迁进 `code/`，**字节守恒**，`evolve-module` 逐字节校验）+
  入口 `code/index.mjs`（只把实体公开面**重导出**：`export *` 的绑定是活的，无业务语义、无写面）。
- `provides:` `idempotency`（实体自述的真实服务键；占位键已改写）。
- 实测：`tools/plugin.sh status system/idempotency-guard` ⇒ `valid:true`、`reason:null`；`load` 真进口（`effects` 非 0）、`unload` 后 `effects_after:0`。
