 ...
 *     → items 40 / counts 15-25-0-40-0/ 三源一致 / 与 `15-requirements-coverage.md` §2 的插件名集合**逐名一致**
 *  14 空集合守卫（一条都没跑 = 红）
 *
...
try {
  ...
  // 都要过白名单 —— 白名单本身**一格没松**，只是把重导链**读穿了**。
  const importLeaks = chain.per_file.flatMap((entry) =>
    entry.specs.filter((spec) => !ALLOWED_IMPORTS.includes(spec) && spec !== entry.link))
